...
from tzrec.ops.scatter_ops import scatter_sum
...
def _jagged_mean(values: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
    ...
    Empty segments pool to zeros: the denominator is clamped, so the
    pooled vector of a candidate-less request is the zero vector rather
    than NaN.
    """
    ...
    sums = scatter_sum(
        flat, jagged_segment_ids(lengths, output_size=flat.size(0)), batch_size
    )
...
class OneRankPredictionHead(BaseModule):
    ...
    Consumes the per-candidate per-task representations ``r^i_k`` produced
    by :class:`~tzrec.modules.gr.onerank_tokenizer.OneRankHSTUTransducer`
    and emits one logit per ``(candidate, task)`` pair (paper 2.3 - 2.4)::
    ...
    for most of a single-epoch run (see the "training dynamics" section of
    ``docs/source/models/dlrm_hstu_onerank.md``).

    Both stages are optional (paper ablations V5 and V3).  With neither,
    ``z_k`` degenerates to the mean of ``{r^i_k}`` over the request's
    candidates, which still discriminates candidates through ``r^i_k``.

    The inner-product scorer replaces ``FusionMTLTower``: the baseline
    funnels every task through one shared MLP and splits only in the final
    linear layer, so the tasks share nearly all capacity.  Here each task
    owns its whole channel ``(z_k, r^i_k)`` and the only shared parameters
    are the STU trunk.
    ...
    def _request_vectors(
        ...
        if sd is None:
            pooled = _jagged_mean(task_embeddings, num_candidates)
    ...
    def forward(
        ...
        # output_size is task_embeddings.size(0) (= sum(num_candidates)),
        # statically known, so the op needs no device->host sync.
        broadcast = torch.repeat_interleave(
            request_vectors,
            num_candidates,
            dim=0,
            output_size=task_embeddings.size(0),
        )
        if self._scorer_type == "dot_product":
            ...
            assert self._task_mlps is not None
