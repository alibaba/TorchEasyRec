...
for DEVICE in cu126
do
    docker build --network host -t ${REGISTRY}/tzrec-test:${DOCKER_TAG}-${DEVICE}-rdma3 --build-arg DEVICE=${DEVICE} .
    docker push ${REGISTRY}/tzrec-test:${DOCKER_TAG}-${DEVICE}-rdma3
done
