...
class DecayAUC(Metric):
    ...
    def __init__(
        self, decay: float = 1.0, thresholds: int = 200, **kwargs: Any
    ) -> None:
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        if torch.any(self.confmat > 0) and 1 > self._decay > 0:
            self.confmat = self._decay * self.confmat + (1 - self._decay) * state
        else:
            self.confmat += state
