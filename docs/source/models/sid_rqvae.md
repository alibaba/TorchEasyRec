...
- sid_rqvae: RQVAE 模型参数 (主输入特征组无需单独配置, 取第一个声明的 feature group)
  ...
  - codebook: 每层码本大小; **列表长度即残差层数 (= SID 的位数)**; 示例 `[256, 256, 256]`，则码本语义空间大小为`256 * 256 * 256 = 16777216`; 支持非均匀如 `[256, 512, 1024]` (每层独立拟合一个 faiss.Kmean; 可类比层级类目——各层码本大小相当于一级/二级/三级类目的个数); **根据业务规模和实际码表碰撞率可对codebook规模进行调整, SID无冲突率(去重SID使用数/商品数)>70%时效果更佳，亿级规模物品集码本参考可设`[8192, 8192, 8192]`**;
