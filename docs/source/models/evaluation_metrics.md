...
**归一化熵（Normalized Entropy, NE）** 是二分类任务中的校准敏感指标，定义为模型的交叉熵除以以样本平均标签率为常数预测器的交叉熵。常与 AUC 配合使用：AUC 衡量排序能力，NE 衡量校准能力。

| 字段名 | 类型  | 是否可选 | 描述                                                           |
| ------ | ----- | -------- | -------------------------------------------------------------- |
| eta    | float | optional | 用于将样本平均标签率从 {0, 1} 处拉离的最小值，默认为 `1e-12`。 |

> ✅ 公式：
> NE = ce_sum / ce_norm，其中
> ce_sum = -Σ [y_i·log(p_i) + (1-y_i)·log(1-p_i)]
> ce_norm = -[N_pos·log(p_avg) + N_neg·log(1-p_avg)]，p_avg 为样本平均标签率
>
> NE < 1 表示模型优于以 p_avg 作为常数预测的基线。
