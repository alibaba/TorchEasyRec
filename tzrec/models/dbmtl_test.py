...
from tzrec.models.dbmtl import DBMTL
...
class DBMTLTest(unittest.TestCase):
    ...
    def test_dbmtl(self, graph_type) -> None:
            ...
            dbmtl=multi_task_rank_pb2.DBMTL(
                ...
                task_towers=[
                    ...
                    tower_pb2.BayesTaskTower(
                        ...
                        relation_mlp=module_pb2.MLP(hidden_units=[8, 3]),
                    ),
                    tower_pb2.BayesTaskTower(
        ...
        init_parameters(dbmtl, device=torch.device("cpu"))
        dbmtl = create_test_model(dbmtl, graph_type)
        ...
        if graph_type == TestGraphType.JIT_SCRIPT:
            predictions = dbmtl(batch.to_dict())
        else:
            predictions = dbmtl(batch)
    ...
    def test_dbmtl_has_sequences(self, graph_type) -> None:
        feature_cfgs = [
            feature_pb2.FeatureConfig(
                id_feature=feature_pb2.IdFeature(
                    feature_name="cat_a", embedding_dim=16, num_buckets=100
