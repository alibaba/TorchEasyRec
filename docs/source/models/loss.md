...
- DlrmHSTU系列模型：模型会发布每个请求的候选数，默认一个请求的候选序列即为一个list，无需额外配置，详见[dlrm_hstu_onerank](dlrm_hstu_onerank.md)；也可以设置`session_name`（如user_id）让list跨越同一个batch内该用户的多个请求
...
1. session_name: list分组的字段名；DlrmHSTU系列模型可不设置（按请求分组），其他模型必须设置
1. temperature_init: softmax温度初始值，logits会乘以 1 / temperature，默认值0.07
1. learnable_temperature: 温度是否可学习（训练中会被clamp防止溢出），默认值true

注意事项：

1. 一个list需要同时含有至少一个正样本和一个负样本才会计入该项损失，被屏蔽的list仍计入分母。使用`session_name`时要求同一个session的样本落在同一个batch内（不要求相邻），样本构造方式与[jrc_loss](#jrc_loss)相同（`DISTRIBUTE BY session_id SORT BY session_id`），并尽量加大`batch_size`；若样本未按session分组，绝大多数list只含一条样本而被屏蔽，表现为该项损失接近0
1. 该损失按list求平均，不支持逐样本权重：配置了`sample_weight_name`或`task_space_indicator_label`的任务不能使用该损失
