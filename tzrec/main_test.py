...
class MainTest(unittest.TestCase):
    ...
    def test_create_custom_model(self) -> None:
        ...
        model_config.custom_model.class_path = "custom.models.CustomModel"
        model_config.custom_model.config.Pack(MLP(hidden_units=[32, 16]))

        with mock.patch("tzrec.main.load_by_path", return_value=BaseModel):
    ...
    def test_create_custom_model_requires_base_model(self) -> None:
        ...
        model_config = ModelConfig()
        model_config.custom_model.class_path = "custom.models.InvalidModel"
        model_config.custom_model.config.Pack(MLP(hidden_units=[32]))

        with (
            mock.patch("tzrec.main.load_by_path", return_value=torch.nn.Linear),
            self.assertRaisesRegex(ValueError, "must inherit BaseModel"),
        ):
