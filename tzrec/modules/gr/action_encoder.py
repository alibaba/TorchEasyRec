...
class ActionEncoder(BaseModule):
    ...
    @property
    def need_watchtime(self) -> bool:
        """Action Encoder need watchtime feature or not."""
        return False
    ...
    def forward(
        ...
        total_uih_len: int,
        total_targets: int,
        seq_watchtimes: Optional[torch.Tensor],
    ) -> torch.Tensor:
        """Forward the module.

        Args:
            seq_actions (torch.Tensor): action sequence embeddings.
            ...
            seq_watchtimes (torch.Tensor): watchtime sequence embeddings.
...
class SimpleActionEncoder(ActionEncoder):
    ...
    def need_watchtime(self) -> bool:
        """Action Encoder need watchtime feature or not."""
        return len(self._watchtime_to_action_thresholds_and_weights) > 0
        ...
        if len(self._watchtime_to_action_thresholds_and_weights) > 0:
            for threshold, weight in self._watchtime_to_action_thresholds_and_weights:
                seq_actions = torch.bitwise_or(
                    seq_actions, (seq_watchtimes >= threshold).to(torch.int64) * weight
                )
        exploded_actions = (
            torch.bitwise_and(
...
def create_action_encoder(
    action_encoder_cfg: Union[module_pb2.GRActionEncoder, Dict[str, Any]],
    **kwargs: Any,
) -> ActionEncoder:
    """Create ActionEncoder."""
    if isinstance(action_encoder_cfg, module_pb2.GRActionEncoder):
        action_encoder_type = action_encoder_cfg.WhichOneof("action_encoder")
        config_dict = config_to_kwargs(getattr(action_encoder_cfg, action_encoder_type))
    else:
    ...
    if action_encoder_type == "simple_action_encoder":
        return SimpleActionEncoder(**config_dict)
    else:
        raise RuntimeError(f"Unknown action encoder type: {action_encoder_type}")
