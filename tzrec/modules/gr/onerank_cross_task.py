...
``A`` defaults to Cascade (``A[k][j] = 1`` iff ``j <= k``) for nested
label families -- e.g. duration bitmasks whose label sets only grow,
each positive implying the ones before it: the labels form one strictly
cumulative chain, so a task may read the tasks it is nested inside and
nothing else.
...
class OneRankCrossTaskAttention(BaseModule):
    ...
    def _pairwise_source(
        self, projected: torch.Tensor, projected_const: torch.Tensor
    ) -> torch.Tensor:
        """Diag-mix projected ``(B, K, D)`` keys/values into ``(B, K, K, D)``.

        Index ``[b, k, j]`` is the vector task ``k`` reads from task
        ``j``: the diagonal keeps ``projected`` (gradient reaches both
        the projection weights and ``h_j``), every off-diagonal entry
        takes ``projected_const`` (gradient reaches the projection
        weights only).  Under Strategic Gradient Detachment the paper
        detaches ``h_j``, not the shared projections, so the constant
        source must be a second projection of the *detached input*, not
        a detached copy of the projected result: detaching the projected
        result would silence every off-diagonal pair for the projection
        weights while leaving the forward bit-identical.
        """
        num_tasks = self._num_tasks
        diagonal = torch.eye(
            num_tasks, dtype=projected.dtype, device=projected.device
        ).view(1, num_tasks, num_tasks, 1)
        with_grad = projected.unsqueeze(1).expand(-1, num_tasks, -1, -1)
        constant = projected_const.unsqueeze(1).expand(-1, num_tasks, -1, -1)
        return with_grad * diagonal + constant * (1.0 - diagonal)
    ...
    def forward(self, request_vectors: torch.Tensor) -> torch.Tensor:
        ...
        # Project once on (B, K, D) and assemble the (B, K, K, D) source
        # afterwards: Kx less projection GEMM than projecting the
        # expansion.  Under detachment the off-diagonal source stays a
        # function of the projection weights while being constant in
        # h_j, so it gets its own GEMM over the detached input (see
        # _pairwise_source); without detachment the two sources are the
        # same tensor, which reproduces the fully-trainable expansion.
        query = self._q_proj(request_vectors).view(
            batch_size, num_tasks, 1, num_heads, head_dim
        )
        keys = self._k_proj(request_vectors)
        values = self._v_proj(request_vectors)
        if self._gradient_detachment:
            keys = self._pairwise_source(keys, self._k_proj(request_vectors.detach()))
            values = self._pairwise_source(
                values, self._v_proj(request_vectors.detach())
            )
        else:
            keys = self._pairwise_source(keys, keys)
            values = self._pairwise_source(values, values)
        keys = keys.view(batch_size, num_tasks, num_tasks, num_heads, head_dim)
        values = values.view(batch_size, num_tasks, num_tasks, num_heads, head_dim)
