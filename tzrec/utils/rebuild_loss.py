...
class ParetoDynamicLossWeight(torch.nn.Module):
    ...
    def forward(
        self, losses: Dict[str, torch.Tensor], model: torch.nn.Module, init_c: float
    ) -> torch.Tensor:
        ...
        c = np.array([init_c] * len(losses), dtype=np.float32).reshape([-1, 1])
        new_w = self._pareto_step(w, c, grads)
        new_loss = []
