...
def jagged_segment_ids(
    ...
    Args:
        ...
        output_size (Optional[int]): total row count ``sum(lengths)`` when
            statically known at the call site; passing it avoids the hidden
            device->host sync (and data-dependent graph break) that
