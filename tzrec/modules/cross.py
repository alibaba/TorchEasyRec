...
class CrossLayer(nn.Module):
    ...
    def forward(self, x0: torch.Tensor, xl: torch.Tensor = None) -> torch.Tensor:
        ...
        Args:
            x0 (torch.Tensor): Original input features with shape (batch_size, input_dim)
            xl (torch.Tensor, optional): Input from previous layer with shape (batch_size, input_dim).
                                       If None, will use x0. Defaults to None.
...
class DCNv2Layer(nn.Module):
    ...
    def forward(self, x0: torch.Tensor, xl: torch.Tensor = None) -> torch.Tensor:
        ...
        linear_part = torch.matmul(v_xl, self.U.T) + self.bias  # (batch_size, input_dim)
