...
class JRCLoss(_Loss):
    ...
    def forward(
        ...
        Args:
            ...
            segment_ids: a `Tensor` with shape [batch_size], session index of
                each sample, for samples in arbitrary order.
        ...
        ge_loss = torch.where(
            is_pos,
            F.softplus(lse_neg.index_select(0, segment_ids) - logits_pos),
            F.softplus(lse_pos.index_select(0, segment_ids) - logits_neg),
        )
