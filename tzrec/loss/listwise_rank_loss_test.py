...
class ListwiseRankLossTest(unittest.TestCase):
    ...
    def test_matches_per_request_reference(
        ...
        for lengths_list, labels_list in _CASES:
            with self.subTest(lengths=lengths_list):
                ...
                if learnable:
                    # The scale gradient too: a module that detached its
                    # own logit_scale (or the whole scaled branch) would
                    # pass every assertion above while learning nothing
                    # about the temperature.  The reference differentiates
                    # against the same log-space parameter the module
                    # exposes, rebuilding its clamp+exp chain.
                    got_scale_grad = torch.autograd.grad(got, module.logit_scale)[0]
                    ref_logit_scale = (
                        module.logit_scale.detach().clone().requires_grad_(True)
                    )
                    ref_scale = ref_logit_scale.clamp(max=math.log(100)).exp()
                    want_scale = _reference_loss(
                        logits, labels, lengths_list, ref_scale
                    )
                    if want_scale.requires_grad:
                        want_scale_grad = torch.autograd.grad(
                            want_scale, ref_logit_scale
                        )[0]
                    else:
                        # All requests dropped: the reference is constant,
                        # and so must the module's scale gradient be.
                        want_scale_grad = torch.zeros_like(got_scale_grad)
    ...
    def test_dropped_requests_leak_no_gradient(self) -> None:
        ...
        # And the value is that of the usable request, spread over the
        # total request count (the module's denominator is *all* requests,
        # so the two dropped ones dilute it by exactly 3).
        detached = logits.detach()
        torch.testing.assert_close(
            module(detached, labels, lengths),
            module(detached[:3], labels[:3], torch.tensor([3])) / 3,
        )
    ...
    def test_empty_batch_is_finite_zero(self) -> None:
        """A zero-request batch (B == 0) must be 0, not 0/0 -> NaN.
        ...
        value = module(logits, torch.tensor([]), lengths)
        self.assertTrue(torch.isfinite(value))
        self.assertEqual(value.item(), 0.0)
        value.backward()
        self.assertTrue(torch.isfinite(module.logit_scale.grad))
    ...
    def test_segment_ids_in_any_order(self) -> None:
        ...
        segment_ids = torch.repeat_interleave(torch.arange(4), lengths)[perm]

        loss = ListwiseRankLoss()
        torch.testing.assert_close(
            loss(logits[perm], labels[perm], lengths, segment_ids),
