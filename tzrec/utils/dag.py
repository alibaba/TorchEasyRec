...
class DAG(object):
    ...
    def add_node_if_not_exists(self, node_name, graph=None):
        """Add a node if it does not exist yet, otherwise do nothing."""
        try:
    ...
    def delete_node_if_exists(self, node_name, graph=None):
        """Delete a node if it exists, otherwise do nothing."""
        try:
    ...
    def all_downstreams(self, node, graph=None):
        """Returns nodes in the dependency graph in topological order."""
        if graph is None:
            graph = self.graph
        nodes = [node]
        nodes_seen = set()
