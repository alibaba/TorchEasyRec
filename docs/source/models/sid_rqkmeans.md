...
RQKMeans (Residual K-Means) 是一种语义ID (Semantic ID, 简称 SID) 生成模型, 和 [SID RQVAE](sid_rqvae.md) 一样把物品 embedding 量化成一串离散编码 `(code_0, code_1, ..., code_{n-1})`, 用作生成式推荐的物品token。区别在于: RQKMeans **用 FAISS K-Means 直接对残差做聚类来得到每层码本**, 没有编码器/解码器 MLP, 也没有 STE/commitment 等梯度训练——码本是离线一次性拟合出来的。
...
embedding 列为原生数组列, 无需 `separator`。示例数据由脚本生成 (与 RQVAE 共用):

```bash
python data/sid_example/gen_sid_example_data.py
# 产出非 CLIP 数据: data/sid_example/item_only/part-0.parquet (item_id + embedding)
```
...
- train_config: K-Means 非梯度训练, 优化器与 epoch 数仅为框架训练循环所需的形式配置
  ...
  - save_checkpoints_steps / save_checkpoints_epochs: **必须。设 `0` 关闭周期性保存; 拟合好的码本只随训练结束的最终 checkpoint 持久化**
- data_config / feature_configs / feature_groups: 同 RQVAE, 但只需主物品 embedding 一组 (`deep`); 其拼接后的总维度即 K-Means 的向量维度 (不单独配置 input_dim)
...
见仓库根目录 `sid_rq_kmeans.config`。

### 数据

[al_sid 物品 embedding 抽样] 通过 `data/sid_example/gen_sid_example_data.py` 生成 `data/sid_example/item_only/*.parquet`。
...
**`OMP_NUM_THREADS=$(nproc) MKL_NUM_THREADS=$(nproc)` 为必须配置项，否则默认设置为1，影响模型训练速度。**
...
OMP_NUM_THREADS=$(nproc) MKL_NUM_THREADS=$(nproc) \
    ...
    -m tzrec.predict \
    --pipeline_config_path sid_rq_kmeans.config \
    --checkpoint_path experiments/sid_rqkmeans/model.ckpt-7 \
...
**`OMP_NUM_THREADS=$(nproc) MKL_NUM_THREADS=$(nproc)` 为必须配置项，否则默认设置为1，影响模型预测速度。**
