...
def save_dataloader_state(
    ...
    Args:
        ...
        checkpoint_state: Local checkpoint state {source_key: max_consumed_row}.
    """
    # All-gather states from all ranks
    if dist.is_initialized():
        world_size = dist.get_world_size()
        all_states = [None] * world_size
        dist.all_gather_object(all_states, checkpoint_state)
...
def restore_dataloader_state(checkpoint_dir: str) -> Optional[Dict[str, int]]:
    ...
    Returns:
        Checkpoint state dict {source_key: max_consumed_row}, or None if not found.
    """
    ckpt_path = os.path.join(checkpoint_dir, DATALOADER_CKPT_FILENAME)
...
def update_checkpoint_state(
    ...
    """Merge batch's checkpoint_info into checkpoint_state by taking max per key.

    This updates checkpoint_state in-place.

    Args:
        checkpoint_state: Accumulated checkpoint state to update.
    ...
    for key, value in checkpoint_info.items():
        if key in checkpoint_state:
            checkpoint_state[key] = max(checkpoint_state[key], value)
        else:
