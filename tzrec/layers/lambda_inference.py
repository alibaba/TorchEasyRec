...
class LambdaOutputDimInferrer:
    ...
    def infer_output_dim(self, 
                        input_dim_info: DimensionInfo, 
                        lambda_fn_str: str,
                        dummy_batch_size: int = 2,
                        dummy_seq_len: Optional[int] = None) -> DimensionInfo:
        """推断lambda表达式的输出维度
        ...
        try:
            ...
            self.logger.error(f"Failed to infer output dim for lambda '{lambda_fn_str}': {e}")
            if self.safe_mode:
                # 安全模式下返回输入维度
    ...
    def _compile_lambda_function(self, lambda_fn_str: str) -> Callable:
        ...
        try:
            ...
            if not callable(lambda_fn):
                raise ValueError(f"Lambda expression does not evaluate to a callable: {lambda_fn_str}")
            
            return lambda_fn
            
        except Exception as e:
            self.logger.error(f"Failed to compile lambda function '{lambda_fn_str}': {e}")
    ...
    def _validate_lambda_safety(self, lambda_fn_str: str) -> None:
        ...
        dangerous_keywords = [
            ...
            'vars', 'dir', 'compile', 'reload'
        ]
        
        lambda_lower = lambda_fn_str.lower()
        for keyword in dangerous_keywords:
            if keyword in lambda_lower:
                raise ValueError(f"Potentially unsafe lambda expression contains '{keyword}': {lambda_fn_str}")
        
        # 检查是否是有效的lambda表达式格式
        if not lambda_fn_str.strip().startswith('lambda'):
            raise ValueError(f"Expression must be a lambda function: {lambda_fn_str}")
    
    def _analyze_output(self, output_tensor: torch.Tensor, input_dim_info: DimensionInfo) -> DimensionInfo:
        ...
        if isinstance(output_tensor, (list, tuple)):
            ...
            return DimensionInfo(
                dim=dims,
                shape=shapes[0] if len(set(shapes)) == 1 else None,  # 如果所有shape相同则保留
                is_list=True,
...
class LambdaLayer(nn.Module):
    ...
    def _infer_output_dim(self):
        ...
        if self._input_dim_info is None:
            raise ValueError("Cannot infer output dimension without input dimension info")
        
        inferrer = LambdaOutputDimInferrer(safe_mode=True)
        self._output_dim_info = inferrer.infer_output_dim(
            self._input_dim_info, 
            self.lambda_fn_str
        )
    ...
    def output_dim(self) -> int:
        ...
        if self._output_dim_info is None:
            raise ValueError(f"Output dimension not available for {self.name}. "
...
def create_lambda_layer_from_input_fn(input_fn_str: str, 
                                     input_dim_info: DimensionInfo,
                                     name: str = "input_fn_layer") -> LambdaLayer:
    """从input_fn字符串创建Lambda层
    
    这个函数可以用于将backbone配置中的input_fn转换为具有output_dim方法的层
    """
    return LambdaLayer(
        lambda_fn_str=input_fn_str,
        input_dim_info=input_dim_info,
        name=name
    )


# 便捷函数
def infer_lambda_output_dim(input_dim_info: DimensionInfo, 
                          lambda_fn_str: str,
                          safe_mode: bool = True) -> DimensionInfo:
