...
- Assume the reader knows PyTorch and TorchRec. Match existing patterns; if uncertain, choose the simpler, more concise implementation. Make the smallest surgical change that solves the request: reuse existing helpers, data structures, and control flow; add no speculative features, abstractions, configurability, fallbacks, or defenses for impossible states; do not refactor or reformat adjacent code; remove only orphans introduced by your change. Keep user-facing docs focused on configuration, operation, observable behavior, and troubleshooting; place internal artifact layouts and implementation contracts beside the code that enforces them.
