...
# decorator on fx_util.py does not cover the call made from here.
torch.fx.wrap(fx_size0_max1)
...
class OneRankListwiseLoss(_Loss):
    ...
    * no negative -- the objective degenerates to "make all scores equal",
    ...
    def forward(
        ...
        # `enable_global_average_loss` rescale in DlrmHSTUOneRank divides by
        # the total count too, so the two denominators agree and DDP's
        # cross-rank gradient average stays an unbiased global mean even
        # when the masked-out fraction differs across ranks.  The max()
        # keeps an empty (zero-request) batch at 0 instead of the NaN of
        # 0/0, which would poison every parameter through the all-reduced
        # gradient.
